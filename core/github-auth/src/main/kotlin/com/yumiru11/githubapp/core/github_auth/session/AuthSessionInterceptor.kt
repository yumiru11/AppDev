 ...
 * 重放请求打 [RETRY_MARKER] 标记，保证**最多重放一次**，防止 401 死循环。
 */
class AuthSessionInterceptor(
    ...
    override fun intercept(chain: Interceptor.Chain): Response {
        ...
        // 已重放过的请求直接放行：最多重放一次
        if (request.tag(RETRY_MARKER::class) != null) {
            return chain.proceed(request)
        }
        ...
        val fresh = runBlocking { refresher.refreshIfNeeded() }
            .getOrNull()
            ?.takeIf { it }
        ...
        val retry =
            response
                .request
                .newBuilder()
                .tag(RETRY_MARKER::class, RETRY_MARKER)
