...
@Composable
fun EnhancedParagraph(model: MarkdownComponentModel) {
    val isSingleHttpImage =
        remember(model.content, model.node) {
            ...
            SINGLE_IMAGE_LINK_REGEX.matches(nodeText) && (src?.startsWith("http") == true)
        }
    if (isSingleHttpImage) {
        // 仅网络图（徽章）提升为块级且保持原始尺寸；本地图段落保持默认内联渲染
        // （MarkdownImage 需要 IMAGE 节点，段落节点会渲染空——2026-08-16 回归风险规避）
        EnhancedMarkdownImage(model, stretch = false)
    } else {
