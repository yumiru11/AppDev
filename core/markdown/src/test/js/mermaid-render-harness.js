 ...
 *    （离线 `pre > code.language-mermaid`、服务端 `div.highlight-source-mermaid > pre`）。
 *    mermaid 引擎本身用**记录型 stub**（真实渲染需要浏览器布局引擎；这里测的是选择/配置/
 *    回退逻辑，真实栅格化由 CI 模拟器截图兜底）。
 * 3. **门禁矩阵**：`window.mermaid` 缺失、语法探针失败（模拟 Chromium < 94 的 class static
 *    block 不支持）、`initialize` 抛异常、`run` 失败（suppressErrors 下 resolve 但不产 SVG）
 *    —— 每条路径都必须回退为普通代码块且 DOM 无损。
 *
...
/** 服务端 HTML 通道形态：`<div class="highlight highlight-source-mermaid"><pre>…` */
...
async function main() {
  ...
  // 2c. 无图页面：不得 initialize/run（性能护栏）
  record = installMermaidStub('success');
