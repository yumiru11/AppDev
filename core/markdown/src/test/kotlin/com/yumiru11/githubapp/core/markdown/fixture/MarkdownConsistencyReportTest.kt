...
class MarkdownConsistencyReportTest {
    ...
    fun reportSummaryCounts_matchCatalogDerivedCounts() {
        ...
        listOf(
            ...
            "**11**" to (webViewOnlyReady == 11),
            "**24**" to (native == 24),
            "**0**" to (unimplemented == 0),
        ).forEach { (needle, ok) ->
    ...
    fun reportSummary_offlineAndServerCoverageCounts_matchCatalog() {
        ...
        assertEquals("服务端 HTML 覆盖数（报告写到 34/35）", 34, serverHtml)
        assertEquals("离线 GFM 覆盖数（报告写到 32/35，2026-09-13 KaTeX + Mermaid 落地后）", 32, offline)
        assertTrue("报告必须写出 34 / 35", text.contains("34 / 35"))
