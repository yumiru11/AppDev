...
- **当前状态**：WebView 兜底已建（T9）；原生渲染（T7 产物）功能缺口待补
- **用户拍板（C3-1）**：**先做 prototype——专门做一个场景只做 README，复刻定下来的完整 README 画面（双版本对照：WebView 现状 vs 原生增强版），用户看完再定路线**
- 路线候选（prototype 后选）：
  - A. WebView 主渲染（主题融合靠 CSS 变量覆盖）
  - B. 原生增强（rikkahub 式：表格工具栏/复制按钮/字号设置/折叠，主题 100% 融合）
  - C. 混合起步 → 向 B 演进（推荐）
- **已定渲染细节**（无论哪条路线都适用）：
  ...
  - 代码块背景：与卡片一致（默认 surfaceContainer，无主题色偏；可开主题色偏开关）（用户拍板）
