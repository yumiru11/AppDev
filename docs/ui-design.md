...
| **Issue / PR 列表与详情的整页加载态** | ⬜ **未替换（有意回退，2 处）** | `IssueStateUi.kt` / `PullRequestStateUi.kt` 不在 `uiSourceExcludes`，2026-09-13（#261 后）复测仍为 **0/19 行已覆盖**——真实原因是**没有单测渲染这两个状态 UI**，而非旧注释所称的「Robolectric 加载的 Composable 不产 JaCoCo 数据」（该工具链结论已由 #261 证伪）。diff 门禁按比例判定，这两处会成为唯一纳入统计的文件 → 必然 0% 失败。**不自行放宽门禁排除名单**，留待单独决策（见 ADR-0008「已知遗留」） |
