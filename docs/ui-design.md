...
| **按钮内 pending**（16~24dp, strokeWidth 2dp） | ⬜ **保持 flat `CircularProgressIndicator`（18 处）** | 官方明令「In very small buttons, use the flat shape since the wavy shape is not as visible at that size」；同理 `LoadingIndicator` 的**默认容器是 48×48dp**（ActiveSize 38dp，alpha18 `LoadingIndicatorTokens` 实测），塞进 18dp 按钮同样不可读 |
